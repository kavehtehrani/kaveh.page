...
export function BlogMeta({ date, readingTime }: BlogMetaProps) {
  return (
    <dd className="flex flex-wrap text-sm font-medium leading-6 text-gray-500 dark:text-terminal-gray md:text-base">
      <time dateTime={date} className="flex items-center justify-center">
        <span className="ml-1.5 md:ml-2">{formatDate(date)}</span>
      </time>
      <span className="mx-2 text-gray-400 dark:text-terminal-bg-lighter">{` | `}</span>
